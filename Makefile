...
build-docker:
	docker run --rm --name "$(build-container-name)" -v "$(shell pwd)":/go/src/$(DAEMON_DIR) -w /go/src/$(DAEMON_DIR) golang:1.9.2 /bin/bash -c "go get -u github.com/golang/dep/cmd/dep; dep ensure; go build -v"
	$(call compress)
